...
### 🔒 절대 규칙 검증 (Gate-1 강제 검증)

> **강제 검증**: Plan 단계 결과물이 다음 단계(Design)로 넘어가기 전에 @architect가 반드시 절대 규칙 준수 여부를 검증해야 합니다.

#### @analyst 절대 규칙 검증 항목
1. **추측 금지 검증**: 기획서 내용을 추측하지 않고 분석했는가?
2. **절대 규칙 위반 허용 금지 검증**: 기획 단계에서 절대 규칙 위반을 허용하지 않았는가?
3. **문서 읽기 생략 금지 검증**: 필수 문서(PROJECT_SUMMARY.md, CODE_STYLE.md 등)를 읽었는가?
4. **모호한 요청 그대로 진행 금지 검증**: 사용자의 모호한 요청을 그대로 작업에 반영하지 않았는가?

#### 검증 실패 시 조치
- **1차 수정 시도**: @analyst에게 수정 요청
- **2차 수정 시도**: 구체적 수정 방법 제시
- **3차 실패 후**: Plan 단계로 롤백, 사용자 문의

#### Gate-1 최종 통과 조건
- [ ] @analyst 절대 규칙 100% 준수
- [ ] 계획 명확성 검증 통과
- [ ] 영향 파일 완전성 검증 통과
- [ ] 위험 요소 및 대응책 적절성 검증 통과
