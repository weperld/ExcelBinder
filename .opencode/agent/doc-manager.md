...
## Workflow (Test 단계 결과물 검증 - Gate-4 준비)

> Test 단계 결과물이 Docs 단계로 넘어가기 전에 @doc-manager가 절대 규칙 준수 여부를 검증합니다.

### Gate-4 검증 항목

#### @tester 절대 규칙 검증 항목
1. **테스트 삭제 금지 검증**: 실패한 테스트를 삭제하여 "통과"로 만들지 않았는가?
2. **버그 은폐 금지 검증**: 발견한 버그를 보고하지 않고 은폐하지 않았는가?
3. **테스트 결과 왜곡 금지 검증**: 실패를 성공으로 보고하지 않았는가?
4. **절대 규칙 위반 코드 테스트 통과 금지 검증**: 절대 규칙을 위반한 코드의 테스트를 통과시키지 않았는가?

#### 검증 실패 시 조치
- **1차 수정 시도**: @tester에게 수정 요청
- **2차 수정 시도**: 구체적 수정 방법 제시
- **3차 실패 후**: Test 단계로 롤백, @tester에게 재테스트 요청
