...
### 🔒 절대 규칙 검증 (Gate-3 강제 검증)

> **강제 검증**: Code 단계 결과물이 다음 단계(Test)로 넘어가기 전에 @tester가 반드시 절대 규칙 준수 여부를 검증해야 합니다.

#### @developer 절대 규칙 검증 항목
1. **`as any` 사용 금지 검증**: 타입 에러 억제가 없는가?
2. **`@ts-ignore` 사용 금지 검증**: 타입 에러 억제가 없는가?
3. **빈 catch 블록 금지 검증**: `catch(e) {}` 빈 블록이 없는가?
4. **데이터 무결성 위반 금지 검증**: 기본값 대신 예외 발생이 있는가?
5. **비동기 패턴 위반 금지 검증**: try-finally로 IsBusy 상태 보장이 있는가?
6. **테스트 삭제 금지 검증**: 실패한 테스트를 삭제하여 "통과"로 만들지 않았는가?
7. **커밋 없이 파일 수정 금지 검증**: 사용자의 명시적 요청 없이 커밋하지 않았는가?

#### 검증 실패 시 조치
- **1차 수정 시도**: @developer에게 수정 요청
- **2차 수정 시도**: 구체적 수정 방법 제시
- **3차 실패 후**: Code 단계로 롤백, @architect에게 재설계 요청

#### Gate-3 최종 통과 조건
- [ ] @developer 절대 규칙 100% 준수
- [ ] 빌드 성공 검증 통과 (컴파일 에러 0개)
- [ ] 코드 스타일 준수 검증 통과
- [ ] 기술 규칙 준수 검증 통과
