...
| 규칙 | 위반 시 조치 |
|------|------------|
| **타입 에러 억제 금지** | `as any`, `@ts-ignore`, `@ts-expect-error` 사용 금지 |
...
| 범주 | 금지 행동 | 예시 |
|------|----------|------|
| **Type Safety** | 타입 에러 억제 | `as any`, `@ts-ignore` 사용 |
...
2. **절대 규칙 위반 허용 금지**: 기획 단계에서 절대 규칙 위반 허용
3. **문서 읽기 생략 금지**: 필수 문서(PROJECT_SUMMARY.md, CODE_STYLE.md 등) 읽기 생략
4. **모호한 요청 그대로 진행 금지**: 사용자의 모호한 요청을 그대로 작업에 반영
...
2. **비동기 패턴 위반 설계 금지**: try-finally로 IsBusy 상태 보장 설계 누락
3. **절대 규칙 위반 설계 금지**: 절대 규칙을 위반하는 아키텍처 설계
4. **기술 규칙 위반 설계 금지**: TECHNICAL_RULES.md 위반 설계
...
**금지 행동**:
1. **`as any` 사용 금지**: 타입 에러 억제 절대 금지
2. **`@ts-ignore` 사용 금지**: 타입 에러 억제 절대 금지
3. **빈 catch 블록 금지**: `catch(e) {}` 빈 블록 절대 금지
4. **데이터 무결성 위반 금지**: 기본값 대신 예외 발생
5. **비동기 패턴 위반 금지**: try-finally로 IsBusy 상태 보장
6. **테스트 삭제 금지**: 실패한 테스트 삭제하여 통과로 만드는 행위
7. **커밋 없이 파일 수정 금지**: 사용자의 명시적 요청 없이 커밋
...
**금지 행동**:
1. **테스트 삭제 금지**: 실패한 테스트 삭제하여 통과로 만드는 행위
2. **버그 은폐 금지**: 발견한 버그를 보고하지 않고 은폐
3. **테스트 결과 왜곡 금지**: 실패를 성공으로 보고
4. **절대 규칙 위반 코드 테스트 통과 금지**: 절대 규칙을 위반한 코드의 테스트 통과
...
**금지 행동**:
1. **절대 규칙 위반 통과 금지**: 절대 규칙을 위반한 코드 리뷰 통과
2. **데이터 무결성 위반 코드 통과 금지**: 기본값 사용 코드 리뷰 통과
3. **빈 catch 블록 코드 통과 금지**: 빈 catch 블록 코드 리뷰 통과
4. **타입 에러 억제 코드 통과 금지**: `as any`, `@ts-ignore` 사용 코드 리뷰 통과
5. **테스트 삭제 코드 통과 금지**: 테스트 삭제 코드 리뷰 통과
...
2. **절대 규칙 위반 문서화 금지**: 절대 규칙을 위반한 내용 문서화
3. **버그 은폐 문서화 금지**: 발견한 버그를 문서에서 은폐
4. **테스트 결과 왜곡 문서화 금지**: 실패를 성공으로 보고하는 문서 작성
...
| 단계 전이 | 검증 에이전트 | 검증 항목 |
|---------|------------|----------|
| Plan → Design | @architect | 계획 명확성, 영향 파일 완전성, 위험 요소 식별, 절대 규칙 준수 |
| Design → Code | @developer | 아키텍처 실현 가능성, 의존성 구현 가능성, 절대 규칙 준수 |
| Code → Test | @tester | 빌드 성공, 코드 스타일, 기술 규칙 준수, 절대 규칙 준수 |
| Test → Docs | @doc-manager | 테스트 가능성, 버그 0개, 절대 규칙 준수 |
| Docs → QA | @reviewer | 문서 완전성, 코드 일치성, 절대 규칙 준수 |
| QA → Review | @coordinator | 절대 규칙 100% 준수, 코드 품질, 버그 0개 |
...
> **참고**: WorkID 생성은 [WORKFLOW_PLANNING.md](./WORKFLOW_PLANNING.md)에 상세히 설명되어 있습니다.
