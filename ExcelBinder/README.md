...
- **🚫 데이터 처리 및 필터링 규칙**: 
  - **전역 행 규칙**: 첫 행 무시, 2행 헤더, 3행 이후 데이터 시작.
  - **`#` 접두사**: 불필요한 컬럼이나 행을 추출 대상에서 제외.
- **⚡ 고성능 비동기 엔진**: 대규모 엑셀 처리 시 UI 프리징을 방지하는 `async/await` 구조와 템플릿/데이터 캐싱 시스템 탑재.
...
> [!TIP]
> **API 키 없이도 AI의 도움을 받을 수 있습니다!**
> 상세한 프롬프트 템플릿과 활용 가이드는 [루트 README.md](../README.md#-외부-ai-도구-활용-prompt-template)를 확인해 주세요.
