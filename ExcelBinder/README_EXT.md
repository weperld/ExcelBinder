...
### 📑 행(Row) 처리 규칙
- **첫 번째 행 무시**: 모든 시트의 첫 번째 행은 내용에 관계없이 항상 무시됩니다. 주석이나 버전 정보 등을 기입하는 용도로 활용할 수 있습니다.
- **두 번째 행 헤더**: 두 번째 행을 데이터의 **헤더(컬럼명)**로 사용합니다. 스키마 생성 및 데이터 매칭의 기준이 됩니다.
- **세 번째 행 이후 데이터**: 실제 데이터는 세 번째 행부터 시작됩니다.
